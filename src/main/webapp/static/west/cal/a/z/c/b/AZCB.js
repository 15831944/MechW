$(document).ready(function () {
    ...
    $("#cal").html("<table id='azcb'></table>");
    let pg = $("#azcb");

    let south = $("#south");
    let currentTabIndex = null;
    $.getJSON("/static/west/cal/a/z/c/b/AZCB.json", function (result) {
        ...
        function azcb2d(d) {
            ...
            let width = azcbSketch.width();
            let height = azcbSketch.height();

            let svg = d3.select("#d2").append("svg")
                .attr("width", width).attr("id", "AZCBSVG").attr("height", height);
            ...
            svg.append("g")
                .append("text").attr("x", 0).attr("y", 0).attr("dy", -5).attr("text-anchor", "middle")
                .append("textPath").attr("xlink:href", "#AZCBSketchF").attr("startOffset", "50%").text("F");
            ...
            ])).attr("id", "AZCBSketchA").classed("sketch", true);
            svg.append("g")
                .append("text").attr("x", 0).attr("y", 0).attr("dy", -5).attr("text-anchor", "middle")
                .append("textPath").attr("xlink:href", "#AZCBSketchA").attr("startOffset", "50%")
                .text(d);
        }

        currentTabIndex = azcbd2d3.tabs('getTabIndex', azcbd2d3.tabs('getSelected'));
        ...
        let lastIndex;
        pg.propertygrid({
            title: "API 520.2-2015 气体安全阀泄放反力计算",
            data: result,
            showHeader: false,
            showGroup: true,
            scrollbarSize: 0,
            ...
            onEndEdit: function (index, row, changes) {
                if ((!jQuery.isEmptyObject(changes)) && (!jQuery.isEmptyObject(changes.value))) {
                    ...
                    if (currentTabIndex === 0) {
                        azcb2d();
                        azcbSketch.off("resize").on("resize", function () {
                            if ($("#azcb").length > 0) {
                                azcb2d();
                            }
                        });
                    ...
                    if (!jQuery.isEmptyObject(rows[0][columns[0][1].field])) {
                        AZCBW = parseFloat(rows[0][columns[0][1].field]);
                    }
                    else {
                        return false;
                    }
                    ...
                    if (!jQuery.isEmptyObject(rows[1][columns[0][1].field])) {
                        AZCBP1 = parseFloat(rows[1][columns[0][1].field]);
                    }
                    else {
                        return false;
                    }
                    ...
                    if (!jQuery.isEmptyObject(rows[2][columns[0][1].field])) {
                        AZCBT = parseFloat(rows[2][columns[0][1].field]);
                    }
                    else {
                        return false;
                    }
                    ...
                    if (!jQuery.isEmptyObject(rows[3][columns[0][1].field])
                        && parseFloat(rows[3][columns[0][1].field]) < AZCBP1) {
                        AZCBP2 = parseFloat(rows[3][columns[0][1].field]);
                    }
                    else if (!jQuery.isEmptyObject(rows[3][columns[0][1].field])
                        && parseFloat(rows[3][columns[0][1].field]) >= AZCBP1) {
                        south.html("背压不能大于等于 " + AZCBP1 + " MPa").css("color", "red");
                        return false;
                    }
                    else {
                        return false;
                    ...
                    if (!jQuery.isEmptyObject(rows[4][columns[0][1].field])) {
                        AZCBM = parseFloat(rows[4][columns[0][1].field]);
                    }
                    else {
                        return false;
                    }
                    ...
                    if (!jQuery.isEmptyObject(rows[5][columns[0][1].field])) {
                        AZCBK = parseFloat(rows[5][columns[0][1].field]);
                    }
                    else {
                        return false;
                    }
                    ...
                    if (!jQuery.isEmptyObject(rows[6][columns[0][1].field])) {
                        AZCBD = parseFloat(rows[6][columns[0][1].field]);
                    }
                    else {
                        return false;
                    }
                    ...
                    let AZCBA = Math.PI * AZCBD * AZCBD / 4;
                    let AZCBP = 1000 * Math.max(AZCBP2, AZCBP1 * Math.pow(2 / (AZCBK + 1), AZCBK / (AZCBK - 1))) - 100;
                    let AZCBF = 129 * AZCBW * Math.sqrt(AZCBK * AZCBT / (AZCBK + 1) / AZCBM) + AZCBA * AZCBP / 1000;

                    south.html(
                        "<span style='color:#444444;'>" +
                        "安全阀泄放反力 F = " + AZCBF.toFixed(2) + " N" +
                        "</span>");
                                    ...
                                    AZCBPayJS.dialog({
                                        ...
                                        onLoad: function () {
                                            ...
                                            $("#payjs_cancel").off("click").on("click", function () {
                                                ...
                                                // 收银台关闭并清空
                                                AZCBPayJS.dialog("close");
                                                AZCBPayJS.dialog("clear");
                                                // 按钮重置
                                                ...
                                                if (status < 1) {
                                                    ...
                                                    $.ajax({
                                                        ...
                                                        success: function (result) {
                                                            ...
                                                            // 支付成功跳转页
                                                            AZCBPayJS.dialog('refresh', '/static/payjs/payjs_success.html');
