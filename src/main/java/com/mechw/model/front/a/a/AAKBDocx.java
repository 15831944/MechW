package com.mechw.model.front.a.a;

public class AAKBDocx {

    private String ribbonName;

    private Double pd;
    private Double t;
    private Double ps;
    private String test;

    private Double c2;
    private Double fai;
    private String std;
    private String name;

    private Double di;
    private Double bri;
    private Double sri;
    private Double thkn;
    private Double h;

    private Double density;
    private Double rel;
    private Double c1;
    private Double relt;
    private Double ot;
    private Double o;
    private Double ot1;
    private Double et;

    private Double pc;
    private Double c;
    private Double thke;
    private Double dout;
    private Double bro;
    private Double sro;

    private Double m;
    private Double thkc;
    private Double thkm;
    private Double thkd;
    private String thkchk;

    private Double k1;
    private Double k2;
    private Double a;
    private Double b;
    private Double beta;
    private Double sai;
    private Double f;
    private Double re;
    private Double oe;
    private Double pe;
    private Double py;
    private Double pck;
    private Double n;
    private Double pk;
    private String pchk;

    private Double eta;
    private Double pt;

    private Double pm;
    private Double mawp;

    private Double ai;
    private Double ao;
    private Double vi;
    private Double mass;

    public AAKBDocx() {
    }

    public AAKBDocx(String ribbonName, Double pd, Double t, Double ps, String test, Double c2, Double fai, String std, String name, Double di, Double bri, Double sri, Double thkn, Double h, Double density, Double rel, Double c1, Double relt, Double ot, Double o, Double ot1, Double et, Double pc, Double c, Double thke, Double dout, Double bro, Double sro, Double m, Double thkc, Double thkm, Double thkd, String thkchk, Double k1, Double k2, Double a, Double b, Double beta, Double sai, Double f, Double re, Double oe, Double pe, Double py, Double pck, Double n, Double pk, String pchk, Double eta, Double pt, Double pm, Double mawp, Double ai, Double ao, Double vi, Double mass) {
        this.ribbonName = ribbonName;
        this.pd = pd;
        this.t = t;
        this.ps = ps;
        this.test = test;
        this.c2 = c2;
        this.fai = fai;
        this.std = std;
        this.name = name;
        this.di = di;
        this.bri = bri;
        this.sri = sri;
        this.thkn = thkn;
        this.h = h;
        this.density = density;
        this.rel = rel;
        this.c1 = c1;
        this.relt = relt;
        this.ot = ot;
        this.o = o;
        this.ot1 = ot1;
        this.et = et;
        this.pc = pc;
        this.c = c;
        this.thke = thke;
        this.dout = dout;
        this.bro = bro;
        this.sro = sro;
        this.m = m;
        this.thkc = thkc;
        this.thkm = thkm;
        this.thkd = thkd;
        this.thkchk = thkchk;
        this.k1 = k1;
        this.k2 = k2;
        this.a = a;
        this.b = b;
        this.beta = beta;
        this.sai = sai;
        this.f = f;
        this.re = re;
        this.oe = oe;
        this.pe = pe;
        this.py = py;
        this.pck = pck;
        this.n = n;
        this.pk = pk;
        this.pchk = pchk;
        this.eta = eta;
        this.pt = pt;
        this.pm = pm;
        this.mawp = mawp;
        this.ai = ai;
        this.ao = ao;
        this.vi = vi;
        this.mass = mass;
    }

    public String getRibbonName() {
        return ribbonName;
    }

    public void setRibbonName(String ribbonName) {
        this.ribbonName = ribbonName;
    }

    public Double getPd() {
        return pd;
    }

    public void setPd(Double pd) {
        this.pd = pd;
    }

    public Double getT() {
        return t;
    }

    public void setT(Double t) {
        this.t = t;
    }

    public Double getPs() {
        return ps;
    }

    public void setPs(Double ps) {
        this.ps = ps;
    }

    public String getTest() {
        return test;
    }

    public void setTest(String test) {
        this.test = test;
    }

    public Double getC2() {
        return c2;
    }

    public void setC2(Double c2) {
        this.c2 = c2;
    }

    public Double getFai() {
        return fai;
    }

    public void setFai(Double fai) {
        this.fai = fai;
    }

    public String getStd() {
        return std;
    }

    public void setStd(String std) {
        this.std = std;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public Double getDi() {
        return di;
    }

    public void setDi(Double di) {
        this.di = di;
    }

    public Double getThkn() {
        return thkn;
    }

    public void setThkn(Double thkn) {
        this.thkn = thkn;
    }

    public Double getH() {
        return h;
    }

    public void setH(Double h) {
        this.h = h;
    }

    public Double getDensity() {
        return density;
    }

    public void setDensity(Double density) {
        this.density = density;
    }

    public Double getRel() {
        return rel;
    }

    public void setRel(Double rel) {
        this.rel = rel;
    }

    public Double getC1() {
        return c1;
    }

    public void setC1(Double c1) {
        this.c1 = c1;
    }

    public Double getRelt() {
        return relt;
    }

    public void setRelt(Double relt) {
        this.relt = relt;
    }

    public Double getOt() {
        return ot;
    }

    public void setOt(Double ot) {
        this.ot = ot;
    }

    public Double getO() {
        return o;
    }

    public void setO(Double o) {
        this.o = o;
    }

    public Double getOt1() {
        return ot1;
    }

    public void setOt1(Double ot1) {
        this.ot1 = ot1;
    }

    public Double getEt() {
        return et;
    }

    public void setEt(Double et) {
        this.et = et;
    }

    public Double getPc() {
        return pc;
    }

    public void setPc(Double pc) {
        this.pc = pc;
    }

    public Double getC() {
        return c;
    }

    public void setC(Double c) {
        this.c = c;
    }

    public Double getThke() {
        return thke;
    }

    public void setThke(Double thke) {
        this.thke = thke;
    }

    public Double getDout() {
        return dout;
    }

    public void setDout(Double dout) {
        this.dout = dout;
    }

    public Double getThkc() {
        return thkc;
    }

    public void setThkc(Double thkc) {
        this.thkc = thkc;
    }

    public Double getThkm() {
        return thkm;
    }

    public void setThkm(Double thkm) {
        this.thkm = thkm;
    }

    public Double getThkd() {
        return thkd;
    }

    public void setThkd(Double thkd) {
        this.thkd = thkd;
    }

    public String getThkchk() {
        return thkchk;
    }

    public void setThkchk(String thkchk) {
        this.thkchk = thkchk;
    }

    public Double getK1() {
        return k1;
    }

    public void setK1(Double k1) {
        this.k1 = k1;
    }

    public Double getK2() {
        return k2;
    }

    public void setK2(Double k2) {
        this.k2 = k2;
    }

    public Double getA() {
        return a;
    }

    public void setA(Double a) {
        this.a = a;
    }

    public Double getB() {
        return b;
    }

    public void setB(Double b) {
        this.b = b;
    }

    public Double getBeta() {
        return beta;
    }

    public void setBeta(Double beta) {
        this.beta = beta;
    }

    public Double getSai() {
        return sai;
    }

    public void setSai(Double sai) {
        this.sai = sai;
    }

    public Double getF() {
        return f;
    }

    public void setF(Double f) {
        this.f = f;
    }

    public Double getRe() {
        return re;
    }

    public void setRe(Double re) {
        this.re = re;
    }

    public Double getOe() {
        return oe;
    }

    public void setOe(Double oe) {
        this.oe = oe;
    }

    public Double getPe() {
        return pe;
    }

    public void setPe(Double pe) {
        this.pe = pe;
    }

    public Double getPy() {
        return py;
    }

    public void setPy(Double py) {
        this.py = py;
    }

    public Double getPck() {
        return pck;
    }

    public void setPck(Double pck) {
        this.pck = pck;
    }

    public Double getN() {
        return n;
    }

    public void setN(Double n) {
        this.n = n;
    }

    public Double getPk() {
        return pk;
    }

    public void setPk(Double pk) {
        this.pk = pk;
    }

    public String getPchk() {
        return pchk;
    }

    public void setPchk(String pchk) {
        this.pchk = pchk;
    }

    public Double getEta() {
        return eta;
    }

    public void setEta(Double eta) {
        this.eta = eta;
    }

    public Double getPt() {
        return pt;
    }

    public void setPt(Double pt) {
        this.pt = pt;
    }

    public Double getPm() {
        return pm;
    }

    public void setPm(Double pm) {
        this.pm = pm;
    }

    public Double getMawp() {
        return mawp;
    }

    public void setMawp(Double mawp) {
        this.mawp = mawp;
    }

    public Double getAi() {
        return ai;
    }

    public void setAi(Double ai) {
        this.ai = ai;
    }

    public Double getAo() {
        return ao;
    }

    public void setAo(Double ao) {
        this.ao = ao;
    }

    public Double getVi() {
        return vi;
    }

    public void setVi(Double vi) {
        this.vi = vi;
    }

    public Double getM() {
        return m;
    }

    public void setM(Double m) {
        this.m = m;
    }

    public Double getBri() {
        return bri;
    }

    public void setBri(Double bri) {
        this.bri = bri;
    }

    public Double getSri() {
        return sri;
    }

    public void setSri(Double sri) {
        this.sri = sri;
    }

    public Double getBro() {
        return bro;
    }

    public void setBro(Double bro) {
        this.bro = bro;
    }

    public Double getSro() {
        return sro;
    }

    public void setSro(Double sro) {
        this.sro = sro;
    }

    public Double getMass() {
        return mass;
    }

    public void setMass(Double mass) {
        this.mass = mass;
    }
}
